...
fn part1(input: &str) -> Result<usize> {
    let commands:Vec<_> = input.lines().map(|ln| ln.parse::<Command>().context(format!("Parsing {}", ln))).collect::<Result<_,_>>()?;
    ...
    for cmd in commands.iter() {
        match cmd {
            Command::Mem(addr, val) => {
                //let val = usize::from_str_radix(&val_s,2)?;
...
fn float_iter(f: usize) -> impl Iterator<Item=usize> {
    let idx: Vec<usize> = iterate(f, |v| v>>1)
        .take(36).enumerate()
        .filter(|(i, v)| v&1>0).map(|(i,_)| i)
        .collect();
    let n = 2usize.pow(idx.len() as u32);
    (0..n).map(move |bits| 
        iterate(bits, |v| v>>1).map(|v| v&1).zip(idx.iter()).map(|(b, i)| b<<i).sum()
    )
}
