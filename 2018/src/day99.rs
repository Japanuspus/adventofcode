...
fn part01_02(d: &str) -> i64 {
    d.chars().zip(d.chars().cycle().skip(1)).map(|(a,b)| if a==b {a.to_digit(10).unwrap() as i64} else {0}).sum()
}

fn part01_03(d: &str) -> i64 {
    let digits = d.chars().map(|a| a.to_digit(10).unwrap() as i64);
    digits.clone().zip(digits.cycle().skip(1)).filter(|(a,b)| a==b).map(|(a,_b)| a).sum()
